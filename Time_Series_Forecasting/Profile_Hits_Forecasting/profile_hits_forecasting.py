# -*- coding: utf-8 -*-
"""
Created on Mon Jun  3 22:27:12 2019
...
df=pd.read_csv('profile_hits.csv')
#df=df.drop(columns="Unnamed: 0.1")
df=df.drop(columns="Unnamed: 0")
df=df.drop(columns="User")
ss=input("Enter The Owner Name To find the No of Profile Hits Prediction: ")
own=df.loc[df['Profile'] == ss]
...
own = own.groupby('Timestamp')['Hits'].sum().reset_index()

own = own.set_index('Timestamp')
own.index
y = own['Hits'].resample('MS').mean()
...
pred_uc.predicted_mean.plot(ax=ax, label='Expected Number Of Profile Hits')
ax.fill_between(pred_ci.index,
                pred_ci.iloc[:, 0],
                pred_ci.iloc[:, 1], color='k', alpha=.25)
ax.set_xlabel('Date')
ax.set_ylabel('Number Of Profile Hits')
plt.legend()
plt.show()
